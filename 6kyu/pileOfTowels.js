...
function sortThePile(pileOfTowels, weeklyUsedTowels, count = 0) {
    if (!weeklyUsedTowels[0]) return pileOfTowels
    if (weeklyUsedTowels[0] >= pileOfTowels.length) return pileOfTowels.sort().reverse()
    let sortedPile = []

    while (weeklyUsedTowels[0] > 0) {

        if (pileOfTowels[pileOfTowels.length - 1] === 'blue') {
            sortedPile.push('blue')
        }
        if (pileOfTowels[pileOfTowels.length - 1] === 'red') {
            sortedPile.unshift('red')
        }
        pileOfTowels.pop()
        weeklyUsedTowels[0]--
    }
    let newOrder = pileOfTowels.concat(...sortedPile)
    weeklyUsedTowels.shift()
    return sortThePile(newOrder, weeklyUsedTowels, count++)

}

console.log(sortThePile(['blue', 'red', 'blue', 'red', 'blue'], [2, 1, 4,5, 2]))

function sortThePileMath(pileOfTowels, weeklyUsedTowels) {

    const mostTowels = Math.max(...weeklyUsedTowels)
    const usedTowels = pileOfTowels.slice(-mostTowels).sort().reverse()
    const unusedTowels = pileOfTowels.slice(0, -mostTowels)

    if (!mostTowels) {
        return pileOfTowels
    }
    return unusedTowels.concat(...usedTowels)
}

console.log(sortThePileMath(['blue','red','blue','red','blue'], [2,1,4,5,2]))