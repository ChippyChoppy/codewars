...
const number = function(busStops) {
    ...
    for (let i = 0; i < busStops.length; i++) {
      allTheFolks += busStops[i][0]
      allTheFolks += busStops[i][1]
    }
    return allTheFolks
  }
  ...
  const number = function(busStops) {
    ...
    for (stop of busStops) {
      allTheFolks += stop.reduce((a,b) => a - b)
    }
    return allTheFolks
  }
  /*
  Notes/Pseudocode
  bus picks up and drops off folks
  array of pairs
    1st el = ppl getting on
    2nd el = ppl getting off
  return num of ppl still on bus after last stop.
